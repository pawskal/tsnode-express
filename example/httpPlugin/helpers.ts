import { IAuthOptions, IRequestArguments, IRequest, IAuthTarget } from ".";
import { IControllerDefinition, IRoutes } from "./interfaces";
...
export class AuthTarget implements IAuthTarget {
  ...
  constructor(req: IRequest, controllers: Map<string, IControllerDefinition>) {
    controllers.forEach((controller: IControllerDefinition, name: string) => {
    ...
    const controller: IControllerDefinition = controllers.get(this.controller)
